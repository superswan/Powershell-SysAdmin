...
```powershell
netsh int ipv6 set prefixpolicy ::ffff:0:0/96 46
```

```powershell
netsh int ipv6 set prefixpolicy ::/0 45
```
...
#### Forcefully open Internet Explorer (Windows 10)
...
```powershell
Get-ADComputer -Filter "Name -like 'PC-*'" | Select-String -Pattern PC-\
```
...
## GDID
>the GDID (Global Device Identifier) is a persistent, 64-bit Windows fingerprint, revealed in a 2026 criminal complaint, assigned by Microsoft's servers during device registration. The identifier is generated by the `wlidsvc` service, stored in the Windows registry, and then consumed by the Connected Devices Platform (`CDPSvc`) to register the device into Microsoft's Device Directory Service, where it is also reported by Delivery Optimization. The GDID persists across OS updates but changes after a Windows reinstall or by using discovered techniques.
